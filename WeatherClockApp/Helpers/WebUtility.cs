...
namespace WeatherClockApp.Helpers
{
    public static class WebUtility
    {
        ...
        /// <returns></returns>
        internal static UrlParameter[] DecodeParam(string parameter)
        {
            ...
            if (i >= 0)
            {
                while ((i < parameter.Length) || (i == -1))
                {
                    ...
                    if (j > i)
                    {
                        ...
                        {
                            retParams[retParams.Length - 1].Value =
                                parameter.Substring(j + 1, parameter.Length - j - 1);
                        }
        ...
        public static string GetParamValue(string urlString, string paramName)
        {
            ...
            var urlParams = WebUtility.DecodeParam(urlString);
            for (var i = 0; i < urlParams.Length; i++)
            {

                if (Equals(paramName.ToLower(), urlParams[i].Name.ToLower()))
